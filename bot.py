...
async def omikuji(interaction: discord.Interaction):
    ...
    try:
        chat_completion = client.chat.completions.create(
            messages=[
                {"role": "system", "content": "あなたは満別花丸という名前の女の子です。明るく元気に、語尾に「～だもん」や「～まる」をつけて喋ります。おみくじの結果に対して、その意味を解説したり、今日をどう過ごすべきか優しくアドバイスしてください。返答は2〜3文で短くまとめてください。"},
                {"role": "user", "content": f"おみくじで「{result}」が出た人にアドバイスしてあげて！"}
            ],
            model="llama3-8b-8192",
        )
        ai_advice = chat_completion.choices[0].message.content
    except Exception as e:
        ai_advice = "エラーになっちゃったまる。でも、きっといい日になるだもん！"
    ...
    # defer()を使っているので followup.send() で送信する
