...
# 3. VOICEVOXエンジンをダウンロードして設置
# URLを最新の「0.15.5」に変更したまる。これは確実に存在するURLだもん！
WORKDIR /opt
RUN wget https://github.com/VOICEVOX/voicevox_engine/releases/download/0.15.5/voicevox_engine-linux-cpu-0.15.5.7z \
    && 7z x voicevox_engine-linux-cpu-0.15.5.7z \
    && mv linux-cpu voicevox_engine \
    && rm voicevox_engine-linux-cpu-0.15.5.7z
